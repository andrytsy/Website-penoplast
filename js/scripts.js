...
$(document).ready(function() {
	...
	$('.hov1').parent().mouseover(function() {
	...
	$('.btn2').click(function() {
		var choice = $(this).parent().siblings('.info').find('.select > select > option:selected').text();
		console.log(choice);
		$('.choice').html(choice);
	});
	$('#btn1').click(function() {
		$('#catalog').find('img').remove();
		$('#catalog .img_catalog').prepend('<img src="/img/c6/a.jpg">');
	});
	$('#btn2').click(function() {
		$('#catalog').find('img').remove();
		$('#catalog .img_catalog').prepend('<img src="/img/c6/b.jpg">');
	});
	$('.button').click(function() {
		...
		if(answer != false)
		{
			...
			$.ajax({
				...
				setTimeout(function(){yaCounter27059845.reachGoal(''+sbt);}, 30); // меняем XXXXXXXXX на номер счетчика
			});
		}
	});
...
function popup2(id, form, h1, h2, btn) { //onClick="popup2('callback', '');"
	$('.popup_overlay2').show();
	$('#'+id).addClass('activePopup');
	if(id == 'request') {
		var def_h1 = 'Оставить заявку';
...
function popup_out2() {
	$('.popup_overlay2').hide();
	$('.popup2').hide();
	$('.popup2').removeClass('activePopup');
	$('body').find('label').removeClass('red');
}
